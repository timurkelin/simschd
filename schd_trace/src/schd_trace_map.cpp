...
#define WAVE_RGB_COLOR(_name, r, g, b) std::make_pair( \
                                          std::string( _name ), \
                                          std::make_tuple(( r ), ( g ), ( b )))

schd_trace_map_c:: schd_trace_map_c(
      void ) {
   ...
   x11_cmap = {
      ...
      WAVE_RGB_COLOR("Indigo", 0x4B, 0x00, 0x82),
      ...
      WAVE_RGB_COLOR("Olive", 0x80, 0x80, 0x00),
      ...
      WAVE_RGB_COLOR("RebeccaPurple", 0x66, 0x33, 0x99 ),
      ...
      WAVE_RGB_COLOR("Teal", 0x00, 0x80, 0x80 ),
...
schd_trace_map_c::crgb_t schd_trace_map_c::rgb_split( const std::size_t rgb_c ) {
...
std::size_t schd_trace_map_c::rgb_comb(  const schd_trace_map_c::crgb_t rgb_s ) {
   return ( std::get<0>( rgb_s ) << 16 ) |
          ( std::get<1>( rgb_s ) <<  8 ) |
          ( std::get<2>( rgb_s ));
}

std::string schd_trace_map_c::rgb2x11name( const std::size_t  rgb_c ) {
   crgb_t rgb_s = rgb_split( rgb_c );

   boost::optional<const cmap_t::value_type &> min_el_p;
   std::size_t                                 min_se = UINT_MAX;

   BOOST_FOREACH( const cmap_t::value_type& cel, x11_cmap ) {
      int dist_r = std::get<0>( cel.second ) - std::get<0>( rgb_s );
      int dist_g = std::get<1>( cel.second ) - std::get<1>( rgb_s );
      int dist_b = std::get<2>( cel.second ) - std::get<2>( rgb_s );
   ...
   return min_el_p.get().first;
}

std::size_t schd_trace_map_c::x11name2rgb( const std::string& rgb_n ) {
   auto rgb_i = x11_cmap.find( rgb_n );

   if( rgb_i != x11_cmap.end()) {
      return rgb_comb( rgb_i->second );
   }
   else {
