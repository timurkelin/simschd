...
#ifndef SCHD_REPORT_INCLUDE_SCHD_TRACE_MAP_H_
#define SCHD_REPORT_INCLUDE_SCHD_TRACE_MAP_H_
...
private:
      ...
      typedef std::tuple<std::string, crgb_t> cmap_t;

      crgb_t      rgb_split( const std::size_t rgb_c );
      std::size_t rgb_comb(  const crgb_t rgb_s );

      std::string  rgb2x11name( const std::size_t  rgb_c );
      std::size_t  x11name2rgb( const std::string& rgb_n );

#define WAVE_RGB_COLOR(_name, r, g, b) std::make_tuple(         \
                                          std::string( _name ), \
                                          std::make_tuple(( r ), ( g ), ( b )))

      static const std::vector<const cmap_t> cmap = {
...
#endif /* SCHD_REPORT_INCLUDE_SCHD_TRACE_MAP_H_ */
