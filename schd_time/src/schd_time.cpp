...
void schd_time_c::init(
   ...
   if( res_p.is_initialized() && ( res_p.get().length() != 0 )) {
      ...
      SCHD_REPORT_INFO( "schd::time" ) << "Time resolution is set to <" << res_sec << " s>";
   }
   else {
      SCHD_REPORT_FATAL( "schd::time" ) << "Time resolution is not set";
   }
   ...
   if( end_p.is_initialized() && ( end_p.get().length() != 0 )) {
      ...
      SCHD_REPORT_INFO( "schd::time" ) << "Finish time is set to <" << end_sec << " s>";
   }
   else {
      SCHD_REPORT_WARNING( "schd::time" ) << "Finish time is not set";
      end_sec = 0.0;
      end_str = "0.0 s";
   }
} // schd_time_c::init(...)
...
double schd_time_c::to_time_sec(
   ...
   if( !done ) {
      SCHD_REPORT_ERROR( "schd::time" ) << "Incorrect format: " << str;
   }
   else if( val <= 0 ) {
      SCHD_REPORT_FATAL( "schd::time" ) << "Incorrect frequency value";
   }

   return val * std::get<1>( known_units_time[unit_str] );
} // schd_time_c::to_time_sec(...)
