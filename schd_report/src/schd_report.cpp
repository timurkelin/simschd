...
void schd_report_c::init(
   ...
   if( !log_p.is_initialized() || ( log_p.get().size() == 0 )) {
      ...
      SCHD_REPORT_INFO( "schd::report" ) << "Log file name is set to <"
                                         << log_p.get()
                                         << ">";
   }
   else {
      SCHD_REPORT_INFO( "schd::report" ) << "Log file has already been set to <"
                                         << sc_core::sc_report_handler::get_log_file_name()
                                         << ">";
   }
   ...
   if( bea_p.is_initialized() )  {
         ...
         BOOST_FOREACH( const severity_t& svr, severity_v ) {
            ...
            if( prm_p.is_initialized() ) {
               ...
               if( act_p.is_initialized()) {
                  ...
                  SCHD_REPORT_ERROR( "schd::report" ) << "Actions are not specified.";
               }
               ...
               if(( actions & sc_core::SC_LOG ) && !log_file ) {
                  SCHD_REPORT_WARNING( "schd::report" ) << "Logging is enabled for msg_type: <"
                                                        << msg_type
                                                        << ">, severity: <"
                                                        << svr.first
      ...
      SCHD_REPORT_INFO( "schd::report" ) << "Bearing is not specified";
   }
   ...
   if( msg_p.get() == "unchanged" ) {
      ...
      SCHD_REPORT_ERROR( "schd::report" ) << "Incorrect handler specification <"
                                          << msg_p.get()
                                          << ">";
   }
