...
SC_HAS_PROCESS( schd::schd_ptree_xbar_c );
schd_ptree_xbar_c::schd_ptree_xbar_c(
...
void schd_ptree_xbar_c::init(
   ...
   if( !src_list_p.is_initialized()) {
      SCHD_REPORT_ERROR( "schd::xbar" ) << name() << " Source list not found";
   }

   std::size_t src_list_size = src_list_p.get().size();

   if( src_list_size == 0 ) {
      SCHD_REPORT_ERROR( "schd::xbar" ) << name() << " Source list is empty";
   }
   ...
   BOOST_FOREACH( const boost_pt::ptree::value_type& src_el, src_list_p.get()) {
      if( !src_el.first.empty()) {
         SCHD_REPORT_ERROR( "schd::xbar" ) << name() <<  " Incorrect structure";
      }
      ...
      if(( !src_name_p.is_initialized() &&
         ...
         SCHD_REPORT_ERROR( "schd::xbar" ) << name()
      ...
      if( src_name_p.is_initialized() ) {
         ...
         try {
            ...
            SCHD_REPORT_ERROR( "schd::xbar" ) << err.what();
         }
         catch( ... ) {
            SCHD_REPORT_ERROR( "schd::xbar" ) << "Unexpected";
         }
      }
      ...
      if( src_map_it->dump_name.size()) {
         schd_dump_buf_c<boost_pt::ptree> *dump_raw_ptr =
               new schd_dump_buf_c<boost_pt::ptree>( src_map_it->dump_name );

         src_map_it->dump_p =
               boost::optional<schd_dump_buf_c<boost_pt::ptree> &>( *dump_raw_ptr );
      }
   ...
   if( !dst_list_p.is_initialized()) {
      SCHD_REPORT_ERROR( "schd::xbar" ) << name() << " Destination list not found";
   }

   std::size_t dst_list_size = dst_list_p.get().size();

   if( dst_list_size == 0 ) {
      SCHD_REPORT_ERROR( "schd::xbar" ) << name() << " Destination list is empty";
   }
   ...
   BOOST_FOREACH( const boost_pt::ptree::value_type& dst_el, dst_list_p.get()) {
      ...
      if( dst_map_it->dump_name.size()) {
         schd_dump_buf_c<boost_pt::ptree> *dump_raw_ptr =
               new schd_dump_buf_c<boost_pt::ptree>( dst_map_it->dump_name );
...
void schd_ptree_xbar_c::exec_thrd( void ) {
      ...
      for( std::size_t n_stat = 0; n_stat < vi.size(); n_stat ++ ) {
         while( vi.at( n_stat )->num_available()) {
            ...
            if( vo.size() == 1 ) { // Single output. Don't check dst
               ...
               if( dst_str_p.is_initialized() && dst_str_p.get() == "broadcast") { // Broadcast to all outputs
               ...
               } else if( dst_list_p.is_initialized()) {
                  ...
                  BOOST_FOREACH( const boost_pt::ptree::value_type& dst_el, dst_list_p.get()) {
                     ...
                     if( !dst_mod_p.is_initialized()) {
                        SCHD_REPORT_ERROR( "schd::xbar" ) << name() << " Incorrect format";
                     }
                     ...
                     if( vo_map_it == vo_map.end()) {
                        SCHD_REPORT_ERROR( "schd::xbar" ) << name() << " Destination not found: " << dst_mod_p.get();
                     }
                     ...
                     if( dst_wr.at( dst_idx )) {
                        SCHD_REPORT_ERROR( "schd::xbar" ) << name() << " Duplicate access: " << dst_mod_p.get();
                     }
                     else {
                  ...
                  SCHD_REPORT_ERROR( "schd::xbar" ) << name() << " Destination not found";
               }
            } // if( vo.size() == 1 ) ... else ...
         } // while( vi.at( n_stat )->num_available())
      } // for( std::size_t n_stat = 0; n_stat < vi.size(); n_stat ++ )
